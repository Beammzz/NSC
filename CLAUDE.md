...
- Monorepo: Flutter frontend (`Frontend/`), Golang backend (`Backend/`), Python AI (`Inference_backend/`)
- Flutter verify: `cd Frontend && flutter analyze && flutter test`
- Go backend verify: `cd Backend && go vet ./... && go test ./...`
- Python AI verify: `cd Inference_backend && ruff check && pytest`
- State management: Riverpod only (never Provider, never setState at feature level)
- Navigation: GoRouter only
- Backend auth: JWT (access 15 min / refresh 30 days)
- Real-time TSL stream endpoint: `/api/v1/stream` (WebSocket)
