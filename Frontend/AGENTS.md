...
| `frontend/lib/features/settings/` | App settings view displaying 3-way theme switcher (`ThemeMode.system`, `ThemeMode.dark`, `ThemeMode.light`), connected Server IP, demo-mode status, and Shorebird OTA update manager (`otaUpdateProvider`, patch version display, download progress, and app restart via `restart_app`); persisted via `shared_preferences` behind `sharedPreferencesProvider` (overridden in `main()` and in tests) |
| `frontend/lib/features/learn/` | Learn tab (segments `เรียนรู้` / `คลังคำศัพท์`): TSL dictionary (searchable, category-grouped, `SignAvatar` keypoint animation with procedural fallback) and lesson roadmap (topics -> perform-the-sign exercises, pass at model confidence >= the exercise's admin-set threshold). `LearnRepository` (HTTP `/api/v1/learn/*` + simulated demo variant fully synced with the 150-word Server Dictionary and 8 starter roadmap topics). The roadmap draws ONLY lesson icons on a meandering path (`_LessonNode`: emoji disc + progress ring, locked topics show a padlock) — topic titles and exercise words are deliberately absent from the page; the whole disc is the tap target and opens `_LessonSheet` (title, `ผ่านแล้ว N จาก M คำ`, one Start button; a locked topic gets the unlock hint and no button). Start runs the topic END TO END as one lesson, resuming at the first unpassed word. A FINISHED topic's button is `ฝึกทำใหม่`, which is a reset, not a replay: it confirms, then `DELETE /api/v1/learn/progress/topic/{id}` clears that topic's progress before starting at index 0 — required, because the practice screen seeds `_passed` from stored progress and would otherwise show the pass card for every word and skip the whole lesson. A failed reset must NOT start the lesson (same skip bug); the sheet surfaces the error and stays put: `/learn/example` (dictionary avatar + admin-written note) -> `/learn/practice` (scanner camera pipeline, one explicit 3-second try per attempt) -> on pass, `pushReplacement` to the next word's example -> after the last word, `/learn/summary` (correct-out-of-attempts). `ExampleArgs`/`PracticeArgs` carry `(topic, index)`, not a bare exercise, because the index is what advances the lesson; steps replace rather than push so backing out never walks through finished words |
...
- The native camera preview mounts only on the scanner tab; full-screen flows outside the tab shell that need it (exercise practice) must set `cameraMountOverrideProvider` while visible and release it on dispose. In demo mode the simulated stream never emits exercise vocabulary, so the practice screen grants a simulated pass after a few detected frames inside the capture window (`useSimulatedStream` only). An attempt is one explicit bounded capture window (button-triggered, `_captureWindow`), not a free-running frame match: the window's best confidence for the target word is posted once when it closes, and a window in which the word never appeared still posts (confidence 0) so the correct-out-of-attempts tally counts misses.
