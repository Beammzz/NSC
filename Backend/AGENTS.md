...
| Path | Owns |
|---|---|
| `backend/cmd/server/` | Server entrypoint: config load, route wiring (`/api/v1/stream`, `/healthz`) |
...
- Endpoints per root API rules: `/api/v1/stream` (WSS, landmark frames), `/api/v1/conversation` (NLP), `/api/v1/speech` (speech recognition).
