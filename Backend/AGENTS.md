...
- `/api/v1/stream` and `/api/v1/conversation` require a valid JWT (any role) via `Authorization: Bearer` header or the `signmind_access` cookie; for the WS stream the token rides the upgrade request.
