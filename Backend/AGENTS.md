...
| `backend/webui/` | Next.js 15 + React 19 static admin web application source code (nav sidebar: Server dashboard, TSL Model [Predictions Log, AI Logs, Settings (Model Upload, Prediction Log Auto-Cleaning, Runtime Model Tuning, Active Model Summary, Guide)], LLM Model [Coming soon], Learning, Dictionary [Dictionary Words (webcam recorder, file upload, th-sl.com link import, preview), Settings (custom label_map.json upload & reindex)], Users; including AuthProvider, login page, parameter documentation, and user management UI) |
